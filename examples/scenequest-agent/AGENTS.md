...
5. 追加確認後も一致しない、または矛盾が解消しない場合は `no_match` とする。
6. 画像、位置情報、場所・作品の制約、利用可能な質問文のすべてが利用できない場合だけ `invalid` とする。どれか1つでも利用できる場合は、その証拠で安全に実行できる検索・説明・回復フローに降級する。
7. スポット、話数・章・場面単位、距離、出典を捏造しない。このカタログに候補がないことを、その場所が別の作品に一度も登場していない証拠として扱わない。
8. 正確な移動距離は、キュレーション済みの証拠またはホストが計測した証拠がある場合だけ伝える。
9. 画像がない場合は、キュレーション済みの撮影位置だけを案内し、動的な視覚微調整を行わない。視覚確認が必要なら再撮影を依頼する。
10. 位置情報がない場合は、距離を表示しない。近隣候補を紹介する場合も、距離順に並べたと説明しない。

4つの結果状態は `matched`、`uncertain`、`no_match`、`invalid` です。音声説明や内部推理には、照合したカタログID、根拠となるアンカー、安全上の注意を必要な範囲だけ使ってよいが、Page 入力とは分けること。Page には構造化した結果を渡す。その結果は以下の厳密な形だけとする。Page はカメラ撮影や GPS 取得を行わないこと。

### Page input contract

- Page 入力のルートフィールドは次の7つだけ: `status`, `workTitle`, `episodeScene`, `storyLine`, `photoGuidance`, `confidenceLabel`, `nearbySpots`。`status` 以外はスキーマ上は省略可能だが、状態別の必須条件に従う。
- 文字数上限は `workTitle` / `episodeScene` / `storyLine` / `photoGuidance` / `confidenceLabel` の順に 64 / 72 / 100 / 80 / 16 Unicode コードポイント。
- `nearbySpots` は最大3件。各要素は `spotId`, `name`, `distanceLabel`, `directionHint` だけを持ち、それぞれの上限は 40 / 48 / 16 / 48 Unicode コードポイント。`distanceLabel` だけは、有効な位置・距離根拠がない場合に空文字列にできる。
- `catalogId`, `anchors`, `safety` などを Page 入力のルートに追加しない。これらは音声説明または内部推理にのみ使う。
- `matched` は `workTitle`, `episodeScene`, `storyLine`, `photoGuidance` がすべて非空。`confidenceLabel` は省略または空にできる。
- `uncertain` は `photoGuidance` だけを必須とし、ほかの説明文字列は省略または空にできる。
- `no_match` は説明文字列を空にできる。位置情報がないときは、近隣要素の `distanceLabel` も空にする。
- `invalid` は他の入力値を破棄し、Page 側の安全な日本語回復文だけを表示する。未知の状態、型違い、上限超過、余分なルートまたは近隣フィールドも同じ回復にする。
