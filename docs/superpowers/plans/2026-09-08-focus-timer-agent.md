...
**Architecture:** Add one self-contained single-file Page project under the Skill assets. Execute its real Page definition in Node with a fake clock and fake interval registry so elapsed time, transitions, duplicate actions, and lifecycle cleanup are verified without wall-clock waits.
...
- [ ] Run `python3 -m unittest tests.test_focus_timer_agent_contract -v`; expect failure because `skills/rokid-aiui-agent/assets/focus-timer-agent/` does not exist.

### Task 2: Implement the minimal Japanese Page project

**Files:**
- Create: `skills/rokid-aiui-agent/assets/focus-timer-agent/AGENTS.md`
- Create: `skills/rokid-aiui-agent/assets/focus-timer-agent/app.js`
- Create: `skills/rokid-aiui-agent/assets/focus-timer-agent/app.json`
- Create: `skills/rokid-aiui-agent/assets/focus-timer-agent/pages/index/index.ink`
