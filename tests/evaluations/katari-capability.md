...
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| Page route and render | AIUI `0.17.0`; `_current` / `_blank` | `app.json.pages`, Page, `onLoad`, `setData` | One route; no permission | [Stable project structure](https://github.com/yodaos-project/AIUI/blob/88e70bb0382525c1a93ef077c2401dcc31a273ce/documentation/0-guide/structure.md) and repository stable examples | AIX opens `pages/index/index.ink` and renders the safe initial state | Missing or malformed query becomes complete `invalid` patch | One patch per instance; no retained resource | STATIC + LOGIC + AIX | PASS | Strict validator, Node-mounted real setup, final AIX `Preview ready.` |
| Bounded Page data | AIUI `0.17.0`; both targets | `schema.data`; pure normalizer | Eleven declared input fields; no permission | [Stable Page authoring source](https://github.com/yodaos-project/AIUI/tree/88e70bb0382525c1a93ef077c2401dcc31a273ce/documentation) | Valid match and four recovery states produce complete safe payloads | Null, array, unknown enum, wrong types, 14/31/fraction/string duration, and over-limit Unicode become `invalid` | No asynchronous work or cleanup | LOGIC | PASS | Executed boundary suite imports the real setup block with Node 24.19.0 |
| Target-specific density | AIUI `0.17.0`; `_current` / `_blank` | CSS `@media (target: ...)` | No permission | [Stable monochrome design system](https://github.com/yodaos-project/AIUI/blob/88e70bb0382525c1a93ef077c2401dcc31a273ce/design/monochrome/design-system-green.md) | `_current` hides detail; `_blank` shows one memory/evidence group | Long names select compact density without overflow | Pure presentation; no cleanup | STATIC + browser; host layers pending | BLOCKED | Source and 30-case gallery pass, but authenticated target switching in Studio/device is unexecuted |
| Host image/location/locale grounding | Rokid host to Agent; before Page | Host-provided image, place, GPS, voice, locale context | Host-owned; Page declares none | Product contract in `AGENTS.md`; exact host integration must be proven in target Studio/runtime | Prompt-contract cases cover agreeing location and anchors plus vision-only sign | GPS-only cluster, conflicts, look-alike, missing context, and retry exhaustion fall back safely | One retry; no Page sensor handle | LOGIC; STUDIO + DEVICE required | BLOCKED | 15 local cases pass; real host delivery, permissions, revocation, and sensor availability are unexecuted |
| Spoken story playback | Rokid host; Japanese/English | Agent speech response; no Page speech API | Host-owned; Page declares none | Product contract in `AGENTS.md`; target host behavior must be measured | Authored stories are 15–30 seconds by contract and one per result | Recovery states speak no story; long-answer request remains bounded | One response; no Page media resource | LOGIC; STUDIO + DEVICE required | BLOCKED | Text length/duration contracts pass; real voice selection, pacing, interruption, and Page coordination are unexecuted |

The two BLOCKED host capabilities cannot be promoted by local text or browser evidence. KATARI uses no Page network, storage, timer, camera, GPS, speech, gesture, key, Widget, or Agent Worker capability.
